...
type Scanner struct {
	r      *bufio.Reader
	ch     rune // for peek
	chSize int  // bytes occupied by ch in the source
	offset int  // byte offset of ch (the lookahead) in the source
...
// LastTokenSpan returns the [start, end) byte offsets in the source of the
// most recently returned token.
func (s *Scanner) LastTokenSpan() (start, end int) {
	return s.lastTokStart, s.lastTokEnd
}
...
func (s *Scanner) Read() (tok token.Token, lit string) {
	...
	s.lastTokStart = s.offset
	tok, lit = s.readToken()
	s.lastTokEnd = s.offset
	return tok, lit
}

func (s *Scanner) readToken() (tok token.Token, lit string) {
	switch {
	case isLetter(s.ch):
		return s.scanIdent()
	case isDigit(s.ch):
