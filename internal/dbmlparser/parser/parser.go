...
import (
	...
	"github.com/JamesTiberiusKirk/godbml/internal/dbmlparser/token"
)
...
type Parser struct {
	...
	// span of the current token in the source (byte offsets)
	tokStart int
	tokEnd   int
...
func (p *Parser) parseRelationship() (*core.Relationship, error) {
	...
	if p.token != token.IDENT && p.token != token.DSTRING {
		return nil, p.expect("(rel from) table.column_name or table.(col, col)")
	}

	rel.From = p.lit
	rel.FromStart = p.tokStart
	rel.FromEnd = p.tokEnd

	if strings.HasSuffix(rel.From, ".") {
		p.next()
		if p.token == token.LPAREN {
			cols, err := p.readCompositeColumns()
			if err != nil {
				return nil, err
			}
			rel.From = rel.From + "(" + strings.Join(cols, ",") + ")"
			rel.FromEnd = p.tokEnd
			p.next()
		}
	} else {
	...
	p.next()
	if p.token != token.IDENT && p.token != token.DSTRING {
		return nil, p.expect("(rel to) table.column_name or table.(col, col)")
	}
	rel.To = p.lit
	rel.ToStart = p.tokStart
	rel.ToEnd = p.tokEnd

	if strings.HasSuffix(rel.To, ".") {
		p.next()
		if p.token == token.LPAREN {
			cols, err := p.readCompositeColumns()
			if err != nil {
			...
			rel.To = rel.To + "(" + strings.Join(cols, ",") + ")"
			rel.ToEnd = p.tokEnd
		}
	}
	return rel, nil
...
func (p *Parser) readCompositeColumns() ([]string, error) {
	p.next()
	var cols []string
	for p.token == token.IDENT || p.token == token.DSTRING {
		cols = append(cols, p.lit)
		p.next()
...
func (p *Parser) parseTable() (*core.Table, error) {
	table := &core.Table{SpanStart: p.tokStart}
	p.next()
	switch p.token {
	...
	table.Name = p.lit
	table.NameStart = p.tokStart
	table.NameEnd = p.tokEnd
	...
	case token.LBRACE:
		...
		for {
			...
			case token.RBRACE:
				table.SpanEnd = p.tokEnd
				return table, nil
			default:
				columnName := p.lit
				columnNameStart := p.tokStart
				columnNameEnd := p.tokEnd
				...
				if currentToken == token.NOTE && p.token == token.COLON {
					...
					column.NameStart = columnNameStart
					column.NameEnd = columnNameEnd
	...
	if p.token == token.LBRACK {
		...
		for {
			...
			case p.token == token.TYPE:
				...
				p.next()
				if p.token != token.IDENT {
					return nil, p.expect("index type identifier (btree|hash|gin|gist|...)")
				}
...
func (p *Parser) parseColumn(name string) (*core.Column, error) {
	...
	switch p.token {
	case token.IDENT, token.DSTRING, token.STRING, token.TSTRING:
		// IDENT for plain types (int, varchar, jsonb, custom enum names);
		// quoted forms for types with special chars like "text[]" or 'jsonb[]'.
	default:
		return nil, p.expect("int, varchar, \"text[]\", ...")
	}
	column.Type = p.lit
	column.TypeStart = p.tokStart
	column.TypeEnd = p.tokEnd
	p.next()
...
func (p *Parser) parseColumnSettings() (*core.ColumnSetting, error) {
	...
	for {
		...
		case token.REF:
			...
			columnSetting.Ref.ToStart = p.tokStart
			columnSetting.Ref.ToEnd = p.tokEnd
		case token.NOT:
			p.next()
...
func (p *Parser) next() {
	for {
		...
		p.tokStart, p.tokEnd = p.s.LastTokenSpan()
